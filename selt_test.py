...
class SeltTest:
    ...
    def huawei_5600_selt_test(host: str, interface_address: str, port: str) -> str:
        ...
        with Telnet(host) as tn_socket:
            ...
            time.sleep(1)
            logger.critical("Deactivating port...")
            tn_socket.write("deactivate ".encode("ascii") + port.encode("ascii") + b"\r\n")
            time.sleep(1)
            try:
                logger.critical("Starting SELT test...")
                tn_socket.write("adsl selt ".encode("ascii") + port.encode("ascii") + b"\r\n\r\n")
                ...
                logger.critical("Activating port...")
                tn_socket.write("activate ".encode("ascii") + port.encode("ascii") + b"\r\n")
                tn_socket.close()
            except Exception as error:
                logger.critical("Test failed, try again :(" + error)
                tn_socket.close()
